...
def find_smallest_sequentially_covering_subset_n3(paragraph, keywords):
	...
	for size in range(len(keywords), len(paragraph)):
		for start_idx in range(len(paragraph) - size):
			keyword_index = 0
				...
				if word == keywords[keyword_index]:
					if keyword_index == 0:
						start = i + start_idx
							...
							(result == Subarray(start=-1, end=-1) or
								i + start_idx - start < result.end - result.start)):
						result = Subarray(start=start, end=i + start_idx)
					keyword_index += 1
	return result
...
print(find_smallest_sequentially_covering_subset_n3(paragraph, keywords))
